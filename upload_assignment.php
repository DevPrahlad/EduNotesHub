...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], $file_path)) {
        $sql = "INSERT INTO assignments (semester_id, subject_id, title, description, file_path) VALUES ('$semester_id', '$subject_id', '$title', '$description', '$file_path')";
        if ($conn->query($sql) === TRUE) {
            $response = [
                'status' => 'alert-success',
                'message' => 'New assignment uploaded successfully'
            ];
        } else {
            $response = [
                'status' => 'alert-danger',
...
<body>
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data" action="">
            ...
            <button type="submit" class="btn btn-success btn-block mt-4">Upload Assignment</button>
        </form>
