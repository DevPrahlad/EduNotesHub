...
<title>EduNotesHub</title>

<link rel="icon" type="image/x-icon" href="Bg3.png">
...
$result_count = mysqli_query($con,"SELECT  COUNT(*) As total_records from questionbnk ORDER BY mid DESC");
...
$result = mysqli_query($con,"SELECT  *from questionbnk ORDER BY mid DESC LIMIT $offset, $total_records_per_page");
?>
<?php
...
<img class="w-full group-hover:scale-110 transform transition-transform img-fixed" alt="" src="question_bank/uploads/<?php echo $row['imgpath']; ?>"download>
...
<img src="https://img.icons8.com/office/96/null/downloads.png"/></span><span class="absolute top-3 z-10 font-semibold right-0 bg-blue-400 px-3 drop-shadow py-0 text-white opacity-90 text-sm">
<?php echo $row['sem']; ?> </span>
</div>
...
<b><?php echo $row['name']; ?></b><br><br><a href="question_bank/video-uploads/<?php echo $row['videopath']; ?>"><center><button class="button2" type="button">
