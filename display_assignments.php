...
$limit = 5; // Number of assignments per page
$page = isset($_POST['page']) ? (int)$_POST['page'] : 1;
$offset = ($page - 1) * $limit;

// Fetch total number of assignments
$sql_count = "
    SELECT COUNT(*) AS total 
    FROM assignments 
    JOIN subjects ON assignments.subject_id = subjects.id 
    JOIN semesters ON assignments.semester_id = semesters.id
";

if ($selected_semester_id) {
    $sql_count .= " WHERE assignments.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql_count .= " AND assignments.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql_count .= " WHERE assignments.subject_id = $selected_subject_id";
}
...
// Fetch assignments based on search criteria and pagination
$sql = "
    SELECT 
        assignments.id, 
        assignments.title, 
...
if ($selected_semester_id) {
    $sql .= " WHERE assignments.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql .= " AND assignments.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql .= " WHERE assignments.subject_id = $selected_subject_id";
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            <?php if ($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <img src="plogo.jpg" class="card-img-top" alt="Assignment Image">
                            <div class="card-body">
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="alert alert-warning" role="alert">No assignments found</div>
                </div>
            <?php endif; ?>
        </div>
