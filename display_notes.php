...
$sql_count = "
    ...
    JOIN semesters ON notes.semester_id = semesters.id
";

if ($selected_semester_id) {
    $sql_count .= " WHERE notes.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql_count .= " AND notes.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql_count .= " WHERE notes.subject_id = $selected_subject_id";
}
...
// Fetch notes based on search criteria and pagination
$sql = "
    SELECT 
        notes.id, 
        notes.title, 
...
if ($selected_semester_id) {
    $sql .= " WHERE notes.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql .= " AND notes.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql .= " WHERE notes.subject_id = $selected_subject_id";
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="alert alert-warning" role="alert">No notes found</div>
                </div>
            <?php endif; ?>
        </div>
