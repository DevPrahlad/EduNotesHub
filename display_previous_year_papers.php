...
$limit = 5; // Number of previous year papers per page
$page = isset($_POST['page']) ? (int)$_POST['page'] : 1;
$offset = ($page - 1) * $limit;

// Fetch total number of previous year papers
$sql_count = "
    SELECT COUNT(*) AS total 
    FROM previous_year_papers 
    JOIN subjects ON previous_year_papers.subject_id = subjects.id 
    JOIN semesters ON previous_year_papers.semester_id = semesters.id
";

if ($selected_semester_id) {
    $sql_count .= " WHERE previous_year_papers.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql_count .= " AND previous_year_papers.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql_count .= " WHERE previous_year_papers.subject_id = $selected_subject_id";
}

$result_count = $conn->query($sql_count);
$total_previous_year_papers = $result_count->fetch_assoc()['total'];
$total_pages = ceil($total_previous_year_papers / $limit);

// Fetch previous year papers based on search criteria and pagination
$sql = "
    SELECT 
        previous_year_papers.id, 
        previous_year_papers.title, 
        previous_year_papers.description, 
        previous_year_papers.file_path, 
        previous_year_papers.upload_date, 
...
if ($selected_semester_id) {
    $sql .= " WHERE previous_year_papers.semester_id = $selected_semester_id";
    if ($selected_subject_id) {
        $sql .= " AND previous_year_papers.subject_id = $selected_subject_id";
    }
} elseif ($selected_subject_id) {
    $sql .= " WHERE previous_year_papers.subject_id = $selected_subject_id";
}
...
<html>
<head>
    <title>Previous Year Papers</title>
     <style>
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <img src="clogo.jpg" class="card-img-top" alt="Note Image">
                            <div class="card-body">
